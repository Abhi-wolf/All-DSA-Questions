#include <iostream>
#include <cmath>
using namespace std;

/*
    We build a table of n rows (1-indexed). We start by writing 0 in the 1st row. Now in every
    subsequent row, we look at the previous row and replace each occurrence of 0 with 01, and each
    occurrence of 1 with 10.

    For example, for n = 3, the 1st row is 0, the 2nd row is 01, and the 3rd row is 0110.
    Given two integer n and k, return the kth (1-indexed) symbol in the nth row of a table of n rows.

    k=1  0
    k=2  0 1
    k=3  0 1 1 0
    k=4  0 1 1 0 1 0 0 1
*/

int kthGrammar(int n, int k)
{
    if (n == 1 && k == 1)
        return 0;

    int mid = pow(2, n - 1) / 2;

    if (k <= mid)
        return kthGrammar(n - 1, k);
    else
        return !kthGrammar(n - 1, k - mid);
}

int main()
{
    cout << "\n Answer = " << kthGrammar(4, 1) << endl;
    cout << "\n Answer = " << kthGrammar(4, 2) << endl;
    cout << "\n Answer = " << kthGrammar(4, 3) << endl;
    cout << "\n Answer = " << kthGrammar(4, 4) << endl;
    cout << "\n Answer = " << kthGrammar(4, 5) << endl;
    cout << "\n Answer = " << kthGrammar(4, 6) << endl;
    cout << "\n Answer = " << kthGrammar(4, 7) << endl;
    cout << "\n Answer = " << kthGrammar(4, 8) << endl;
    return 0;
}