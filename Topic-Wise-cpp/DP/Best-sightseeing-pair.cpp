#include <iostream>
#include <vector>
using namespace std;

/* ou are given an integer array values where values[i] represents the value of the ith sightseeing spot.
Two sightseeing spots i and j have a distance j - i between them.

The score of a pair (i < j) of sightseeing spots is values[i] + values[j] + i - j: the sum of the values
of the sightseeing spots, minus the distance between them.
Return the maximum score of a pair of sightseeing spots.

Example 1:
Input: values = [8,1,5,2,6]
Output: 11
Explanation: i = 0, j = 2, values[i] + values[j] + i - j = 8 + 5 + 0 - 2 = 11

Example 2:
Input: values = [1,2]
Output: 2

Constraints:
2 <= values.length <= 5 * 104
1 <= values[i] <= 1000*/

int maxScoreSightseeingPair(vector<int> &values)
{
    int size = values.size();
    int maxScore = 0;

    // The left score at the first index is just the value of the first
    int maxLeftScore = values[0];

    for (int i = 1; i < size; i++)
    {
        int currrightscore = values[i] - i;

        // Update the maximum score by combining the best left score so far
        // with the current right score.
        maxScore = max(maxScore, maxLeftScore + currrightscore);
        int currleftscore = values[i] + i;

        // Update the maximum left score up to the current index.
        maxLeftScore = max(maxLeftScore, currleftscore);
    }
    return maxScore;
}

int main()
{
    vector<int> values = {8, 1, 5, 2, 6};
    cout << "Max Score: " << maxScoreSightseeingPair(values) << endl;
    return 0;
}